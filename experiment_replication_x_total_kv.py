...
def main(strategy, sparsing_factor, destination, random_state=42):
    results_path = global_variables_experiment.get_base_results_path(
        'replication/kv')
    table_iii_columns = ['load_pattern', 'exp_type', 'regression_method',
    ...
    for trace in global_variables_experiment.TRACES[trace_family]:
        for y_metric in global_variables_experiment.Y_METRICS[trace_family]:
            x, y = dataset_management.parse_traces(
                trace, y_metric, ['X_flow.csv', 'X_port.csv', 'X_cluster.csv'])
            x, y = strategy.sparse(x, y, sparsing_factor)

            kv_experiment = experiment.run_experiment(
    ...
    results.to_csv(f'{results_path}/{destination}/kv_table_iii_compared.csv')
