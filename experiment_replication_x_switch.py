...
from sparsing import SparseStrategy
...
def main(strategy: SparseStrategy, sparsing_factor, destination, random_state=42):
    results_path = global_variables_experiment.get_base_results_path(
        'replication/switch')
    table_vii_viii_columns = ['load_pattern', 'exp_type',
                              'regression_method', 'trace_family', 'y_metric', 'switch', 'nmae']
    ...
    for trace_family, traces in global_variables_experiment.TRACES.items():
        for trace in traces:
            for y_metric in global_variables_experiment.Y_METRICS[trace_family]:
                x_trace, y = dataset_management.parse_traces(
                    trace, y_metric, ['X_port.csv'])
                x_trace, y = strategy.sparse(x_trace, y, sparsing_factor)
                ...
                for switch in per_switch_traces.keys():
                    switch_experiment = experiment.run_experiment(
                        per_switch_traces[switch], y, y_metric, random_state=random_state)
