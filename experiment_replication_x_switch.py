...
def main():
        ...
        for trace in traces:
            for y_metric in global_variables_experiment.Y_METRICS[trace_family]:
                for random_state in range(2 if global_variables_experiment.IS_LOCAL else 18):
                    ...
                    switch_experiment = experiment.run_experiment(x, y, y_metric, random_state=random_state)
                    ...
                    for regression_method in ['reg_tree', 'random_forest']:
                        results = pd.concat([
                            ...
                            pd.DataFrame([{'random_state': random_state,
                                           ...
                                           'training_time': switch_experiment[regression_method]['training_time']}])], 
                            ignore_index=True)
    ...
    results.to_csv(f'{results_path}/switch_table_vi_entries.csv')
    ...
    results.to_csv(f'{results_path}/switch_table_vi_compared.csv')
