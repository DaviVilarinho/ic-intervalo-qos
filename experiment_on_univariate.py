...
NROWS = 10 if IS_LOCAL else None
    ...
    for trace in traces:
        for y_metric in y_metrics[trace_family]:
            for k in range(1,16+1):
                ...
                x_minimal_train, x_minimal_test, y_minimal_train, y_minimal_test = train_test_split(minimal_dataset, y_dataframe, test_size=0.7, random_state=42)
                ...
                tables[y_metric] = pd.concat([tables[y_metric], new_minimal_prediction(k, trace_split[1],trace_split[2], "RegTree", y_metric , nmae(pred_reg_tree_minimal, y_minimal_test[y_metric]), training_minimal_reg_tree_time)], ignore_index=True)
                ...
                tables[y_metric] = pd.concat([tables[y_metric],new_minimal_prediction(k, trace_split[1],trace_split[2], "RandomForest", y_metric , nmae(pred_random_forest_minimal, y_minimal_test[y_metric]), random_forest_minimal_training_time)], ignore_index=True)
