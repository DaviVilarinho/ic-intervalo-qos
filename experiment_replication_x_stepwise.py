...
from arg_parser import parse_strategy_and_sparsing_factor
    ...
    while True:
        ...
        for feature in features_available:
            ...
            x_train_minimal, x_test_minimal, y_train_minimal, y_test_minimal = train_test_split(
                x_trace_minimal, y_dataset, test_size=experiment.TEST_SIZE, random_state=random_state)

            regressor.fit(x_train_minimal, y_train_minimal[y_metric])
            pred_regressor = regressor.predict(x_test_minimal)

            nmae_appending_feature_to_the_combination[feature] = experiment.nmae(
                pred_regressor, y_test_minimal[y_metric])
        ...
        print(
            f'Appending {lowest_nmae_from_appending} because the NMAE with it is {nmae_appending_feature_to_the_combination[lowest_nmae_from_appending]} < {old_nmae} (old).')
        ...
        for trace in traces:
            for y_metric in global_variables_experiment.Y_METRICS[trace_family]:
                ...
                for regression_method in ['reg_tree', 'random_forest']:
                    ...
                    minimal_dataset = stepwise_selection(x, y, y_metric, DecisionTreeRegressor() if regression_method == 'reg_tree' else RandomForestRegressor(
                        n_estimators=experiment.RANDOM_FOREST_TREES, random_state=random_state, n_jobs=-1))

                    stepwise_experiment = experiment.run_experiment(
