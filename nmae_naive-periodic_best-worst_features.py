...
EXPERIMENT = "nmae_naive-periodic_best-worst_features"
...
with open(TOTAL_X_FILE_PATH, 'w') as f:
    f.write(f'carga,apps,y,método,período,tipo x,período que ocorreu,x,nmae,\n')
        ...
        for y_metric in y_metrics[trace_family]:
            ...
            for worst_metric in ['5_eth1_rxkB.s',
                ...
                for period in PERIODS:
                    ...
                    x_feature = pd.DataFrame(x_filtered[worst_metric])

                    x_train, x_test, y_train, y_test = train_test_split(
                        x_feature, y_filtered, test_size=TEST_SIZE, random_state=RANDOM_STATE)
                    ...
                    with open(TOTAL_X_FILE_PATH, 'a') as f:
                        f.write(
                            f'{trace_load},{trace_apps},{y_metric},RT,{period},worst_100,{worst_100_in},{worst_metric},{nmae(regression_tree_regressor.predict(x_test), y_test[y_metric])},\n')
                        f.write(
                            f'{trace_load},{trace_apps},{y_metric},RF,{period},worst_100,{worst_100_in},{worst_metric},{nmae(random_forest_regressor.predict(x_test), y_test[y_metric])},\n')
            ...
            for best_metric in [
                ...
                for period in PERIODS:
                    ...
                    x_feature = pd.DataFrame(x_filtered[best_metric])
                    ...
                    with open(TOTAL_X_FILE_PATH, 'a') as f:
                        f.write(
                            f'{trace_load},{trace_apps},{y_metric},RT,{period},best,{best_100_in},{best_metric},{nmae(regression_tree_regressor.predict(x_test), y_test[y_metric])},\n')
                        f.write(
                            f'{trace_load},{trace_apps},{y_metric},RF,{period},best,{best_100_in},{best_metric},{nmae(random_forest_regressor.predict(x_test), y_test[y_metric])},\n')
